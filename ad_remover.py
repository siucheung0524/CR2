...
PROGRAM_JINGLE_KEYWORDS = [
    ...
    "鼓仔", "聽眾的鼓仔", "分享聽眾", "收到的鼓仔", "你不懂,我懂", "你不懂我懂", "你不懂，我懂",
...
2. 【四個段落（Parts 1~4）的起止點判定規範】：
   - 【Part 1（約 10:00~10:30）】：
     * 起點：開播前天氣/廣告結束，節目專屬開場 Jingle 響起之處（通常在 120s~250s）。
     * 終點：10:30 商業廣告與報時開始前。若主持人在話題結束後播了一首流行歌再入廣告，歌曲必須連貫保留，切口落在歌曲結束、廣告/報時開始的一剎那（切勿在主持人說「陣間再講」時就提前截斷！）。
   - 【Part 2（約 10:30~11:00）】：
     * 起點：10:30 廣告結束後，主持人說話或音樂接回之處（約 1650s~1700s）。
     * 終點：【極度重要】必須依據逐字稿實際語意！主持人們在 11:00 前結束話題的時間各集皆不同（可能在 2980s 結束，也可能一直熱聊到 3180s）。【絕對不可在主持人和嘉賓還在聊天途中提前截斷】！中間若有播放歌曲（如 10:45 播歌），必須完整包含在 Part 2 內，不得把 Part 2 拆碎！只有當主持人明確結束話題，隨後緊接著商業廣告、宣傳聲帶或 11:00 新聞報時之處，才是 Part 2 終點！
   - 【Part 3（約 11:00~11:30）】：
     * 起點：11:00 整點新聞與後續商業廣告結束，下半場節目 Jingle 或主持對話重新開始之處（約 3650s~3780s）。
     * 終點：【極度重要】11:30 商業廣告與報時開始前。若主持話題結束後緊接著播放《口水多過浪花》（「天天三點當放假」、「鄭裕玲」、「余德丞」）等電台節目宣傳聲帶或商業廣告，這屬於非節目破口，【必須在主持人說完最後一句話的瞬間立即結束切斷】，絕不可留入正片！
   - 【Part 4（約 11:30~12:00）】：
     * 起點：【極度重要】11:30 廣告破口通常包含《此時此刻 有誰共鳴》（公益團體呼籲如捐款5元支持某協會）等商業/公益宣傳，【全部屬於廣告破口，必須完全切除】！Part 4 起點必須精確落在單元專屬 Jingle（如 Everyday is 新day、雞仔好人、衰佬CLS 等）起拍響起的瞬間，【絕不可延遲 20~30 秒才切入，也絕不可將前面的有誰共鳴公益聲帶保留進來】！
     * 終點：【極度重要】節目最後主持人與嘉賓互相道別、感謝（例如「多謝嘉賓，下星期見，拜拜！」）說完最後一句話的瞬間立即切斷！後續播出的商業廣告（如汽車保養/陳列室、歌手新單曲、驗車公司宣傳、叱咤903台呼）以及 12:00 整點新聞，【全部屬於破口，絕對不可留入正片】！

【嚴格警告】：下方 JSON 範例中的時間數字僅為示範資料結構，【絕不可直接複製抄襲數字】！每一天錄音的起止時間戳都有差異，必須嚴格依據上方提供的逐字稿實際時間戳！

請仔細閱讀前文後理，輸出【保留正片區間 (keep_intervals)】。
請嚴格輸出純 JSON 格式，格式如下：
{{
  "keep_intervals": [
    {{"start": 232.0, "end": 1450.0, "description": "Part 1: 開場 Jingle 至 10:30 歌曲結束、商業破口開始前"}},
    {{"start": 1685.0, "end": 3180.0, "description": "Part 2: 10:30 破口後接回至 11:00 前主持對話真正結束處（完整包含中間歌曲，不得切碎）"}},
    {{"start": 3698.0, "end": 4958.8, "description": "Part 3: 11:00 新聞後 Jingle 接回至 11:30 廣告前（切除後續口水多過浪花宣傳與報時）"}},
    {{"start": 5410.0, "end": 6692.0, "description": "Part 4: 11:30 單元 Jingle 起拍至節目完結道別結束（精準切除有誰共鳴公益宣傳、後續商業廣告與12:00新聞）"}}
  ]
}}
